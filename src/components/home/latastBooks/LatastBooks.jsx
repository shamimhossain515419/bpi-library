...
const LatastBooks = () => {
  const { data: prodcuts, isLoading, error } = useGetAllboksQuery("");
  console.log(error);
  const newLatastBooks = prodcuts?.data || [];
  return (
    <div className=" py-5">
      <Container>
        <div>
        ...
        <div className=" py-5 ">
          <div className="  grid sm:grid-cols-2  lg:grid-cols-4 gap-4 p-10 rounded-2xl border border-[#0000003c]">
            <div className=" col-span-1">
              {newLatastBooks?.slice(0, 4).map((item, i) => (
                <SecondaryCart books={item} key={i} />
              ))}
            </div>
            <div className="col-span-2 hidden lg:block">
              <div className=" flex items-center gap-2 border-x border-[#0000003c] px-2">
                {newLatastBooks?.slice(4, 6).map((product, i) => (
                  <div key={i}>
            ...
            <div className=" col-span-1">
              {newLatastBooks?.slice(6, 10).map((item, i) => (
                <SecondaryCart books={item} key={i} />
              ))}
            </div>
          </div>
